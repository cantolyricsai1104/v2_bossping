...
export const QuestionFormProps = z.object({
  title: z.string(),
  fields: z.array(QuestionFieldSchema),
});

export function QuestionForm({ title, fields }: z.infer<typeof QuestionFormProps>) {
  ...
  const handleSubmit = () => {
    ...
    setIsSubmitted(true);
    // In a real CopilotKit environment, you'd want to return this data to the LLM.
    // CopilotKit handles this automatically if you use useCopilotAction with a render function,
    // or you can inject a function into the context to append a message.
  };
...
export function CopilotFormAgent() {
  useComponent({
    name: "askUserQuestions",
    description: "Render a form to ask the user for more specific details",
    parameters: QuestionFormProps,
    render: QuestionForm
  });
